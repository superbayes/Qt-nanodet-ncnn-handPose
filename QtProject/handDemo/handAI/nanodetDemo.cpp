    ...
    float& scale)
{
    ...
    cv::Mat inputModel;
    halfWpad = abs((float)resizeMat.cols - dst_size) / 2;
    halfHpad = abs((float)resizeMat.rows - dst_size) / 2;
    
    cv::copyMakeBorder(resizeMat, inputModel, floor(halfHpad), ceil(halfHpad), floor(halfWpad),ceil(halfWpad), cv::BORDER_CONSTANT, cv::Scalar::all(0));
...
std::vector<BoxInfo> NanoDet::detect(cv::Mat image, float score_threshold, float nms_threshold)
{
    ...
    preprocess(image, (float)this->input_size, input, halfWpad, halfHpad, scale);
...
{
    static const char* class_names[] = { "person", "hands", "car", "motorcycle", "airplane", "bus",
