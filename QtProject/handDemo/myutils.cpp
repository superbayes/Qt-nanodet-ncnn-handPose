...
bool MyUtils::makeSquarePic(cv::Mat &src, cv::Mat &dst,int &xORy, int &addNum)
{
    ...
    if(wid>hei){
        xORy = 0;
        addNum = wid-hei;
        cv::copyMakeBorder(src,dst,0,addNum,0,0,cv::BORDER_CONSTANT,cv::Scalar::all(0));
    }else if (wid<hei) {
        xORy = 1;
        addNum = hei-wid;
        cv::copyMakeBorder(src,dst,0,0,0,addNum,cv::BORDER_CONSTANT,cv::Scalar::all(0));
    }else {
        xORy = -1;
        addNum=-1;
        dst = src;
